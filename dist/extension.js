...
var MarkdownParser = class {
  ...
  constructor() {
    ...
    this.handlers = {
      "strong": (node2, _, text5, push2) => this.processBold(text5, push2, node2.position.start.offset, node2.position.end.offset),
      "emphasis": (node2, _, text5, push2) => this.processItalic(text5, push2, node2.position.start.offset, node2.position.end.offset),
      "delete": (node2, _, text5, push2) => this.processStrikethrough(text5, push2, node2.position.start.offset, node2.position.end.offset),
      "heading": (node2, _, text5, push2) => this.processHeading(node2, text5, push2, node2.position.start.offset, node2.position.end.offset),
      "link": (node2, _, text5, push2) => this.processLink(text5, push2, node2.position.start.offset, node2.position.end.offset),
      "image": (node2, ancestors, text5, push2) => this.processImage(node2, text5, push2, node2.position.start.offset, node2.position.end.offset, ancestors),
      "inlineCode": (node2, _, text5, push2) => this.processInlineCode(text5, push2, node2.position.start.offset, node2.position.end.offset),
      "code": (node2, _, text5, push2) => this.processCodeBlock(node2, text5, push2, node2.position.start.offset, node2.position.end.offset),
  ...
  **/
  parse(text5) {
    ...
    const pushRange = (start, end, type, opts = {}) => {
    ...
    try {
      const ast = this.processor.parse(text5);
      visitParents(ast, (node2, ancestors) => {
  ...
  processInlineCode(text5, pushRange, start, end) {
    ...
    let closeLen = 0;
    while (end - 1 - closeLen >= start + openLen && text5[end - 1 - closeLen] === "`") {
      closeLen++;
    }
  ...
  processTable(node2, text5, pushRange, start, end) {
    ...
    const visualLength = (str) => {
      ...
      for (const char of str) {
        ...
        len += cp > 65535 ? 2 : cp > 255 ? 2 : 1;
      }
      return len;
    };
    const getVisibleText = (node3) => {
    ...
    const colWidths = [];
    const visibleTexts = /* @__PURE__ */ new Map();
    ...
    const firstNewlineIdx = text5.indexOf("\n", start);
    const secondNewlineIdx = firstNewlineIdx !== -1 ? text5.indexOf("\n", firstNewlineIdx + 1) : -1;
    ...
    node2.children.forEach((row, rowIndex) => {
      const rowStart = row.position?.start?.offset;
      const rowEnd = row.position?.end?.offset;
      if (rowIndex > 0) {
        if (rowStart !== void 0 && rowEnd !== void 0) {
          const rawLine = text5.substring(rowStart, rowEnd);
          if (!rawLine.includes("|")) {
            return;
          }
        }
      ...
      row.children.forEach((cell, i) => {
        if (!cell.position || cell.position.start.offset === void 0 || cell.position.end.offset === void 0) {
          return;
        }
        ...
        if (cell.children.length > 0) {
          ...
          if (firstChildPos && lastChildPos && firstChildPos.start.offset !== void 0 && lastChildPos.end.offset !== void 0) {
            innerStart = firstChildPos.start.offset;
            innerEnd = lastChildPos.end.offset;
          }
        } else {
          const rawText = text5.substring(cellStart, cellEnd);
  ...
  processListItem(node2, text5, ancestors, pushRange, start, end) {
    ...
    const markerStart = i;
    while (i < end && (text5[i] === "*" || text5[i] === "-" || text5[i] === "+")) {
      i++;
    }
    const markerEnd = i;
    if (markerEnd > markerStart) {
      if (i < end && (text5[i] === " " || text5[i] === "	")) {
        i++;
      }
      ...
      const bulletType = `ul_bullet_${levelIndex}`;
...
var ParserManager = class {
  ...
  lastRequestId = 0;
  onDidParseEmitter = new vscode.EventEmitter();
  onDidParse = this.onDidParseEmitter.event;
  constructor() {
  ...
  initWorker() {
    try {
      const workerPath = path2.join(__dirname, "parserWorker.js");
  ...
  getRanges(document3) {
    const cached = this.cache.get(document3.uri.toString());
    if (cached && cached.version === document3.version) {
      return cached.ranges;
    }
    this.parse(document3);
    return void 0;
  }
  parseTimeout;
  parse(document3) {
    ...
    this.parseTimeout = setTimeout(() => {
      ...
      if (this.worker) {
        ...
        const ranges = this.parser.parse(text5);
        this.cache.set(document3.uri.toString(), { version: document3.version, ranges });
        this.onDidParseEmitter.fire({ uri: document3.uri, ranges });
      }
    }, 300);
  }
  dispose() {
...
var DecorationManager = class {
  ...
  registerDecorations() {
    ...
    this.decorationTypes.set("ul_bullet_2", vscode2.window.createTextEditorDecorationType({
      before: {
        contentText: "\u25CB",
        color: new vscode2.ThemeColor("editor.foreground"),
        textDecoration: "none; display: inline-block; vertical-align: middle; margin-right: 0.5em;"
    ...
    this.decorationTypes.set("ul_bullet_3", vscode2.window.createTextEditorDecorationType({
      before: {
        contentText: "\u25A0",
        color: new vscode2.ThemeColor("editor.foreground"),
        textDecoration: "none; display: inline-block; vertical-align: middle; margin-right: 0.5em;"
    ...
    this.decorationTypes.set("ul_bullet_4", vscode2.window.createTextEditorDecorationType({
      before: {
        contentText: "\u25A1",
        color: new vscode2.ThemeColor("editor.foreground"),
        textDecoration: "none; display: inline-block; vertical-align: middle; margin-right: 0.5em;"
    ...
    this.decorationTypes.set("heading1", vscode2.window.createTextEditorDecorationType({
      fontWeight: "bold",
      color: new vscode2.ThemeColor("symbolIcon.classForeground"),
    ...
    this.decorationTypes.set("heading2", vscode2.window.createTextEditorDecorationType({
      fontWeight: "bold",
      color: new vscode2.ThemeColor("symbolIcon.classForeground"),
      textDecoration: "none; font-size: 1.5em !important;"
    }));
    this.decorationTypes.set("heading3", vscode2.window.createTextEditorDecorationType({
      fontWeight: "bold",
      color: new vscode2.ThemeColor("symbolIcon.classForeground"),
      textDecoration: "none; font-size: 1.17em !important;"
    }));
    this.decorationTypes.set("heading4", vscode2.window.createTextEditorDecorationType({
      fontWeight: "bold"
    }));
    this.decorationTypes.set("heading5", vscode2.window.createTextEditorDecorationType({
      fontWeight: "bold",
      textDecoration: "none; font-size: 0.83em !important;"
    }));
    this.decorationTypes.set("heading6", vscode2.window.createTextEditorDecorationType({
      before: {
        contentText: "H6",
        fontWeight: "bold",
        color: "#888888",
  ...
  getDynamicTableCellDecoration(diff, align, empty2, isHeader) {
    ...
    const borderStyle = isHeader ? "none; padding-bottom: 6px; box-sizing: border-box; vertical-align: middle; font-weight: bold;" : "none; padding-bottom: 0px; box-sizing: border-box; vertical-align: middle;";
    const spacer = (n) => ({
      contentText: "\xA0".repeat(n),
      textDecoration: borderStyle
    });
    const decOpts = {
      textDecoration: borderStyle,
      ...empty2 && { color: "transparent" },
      ...leftPad > 0 && { before: spacer(leftPad) },
      ...rightPad > 0 && { after: spacer(rightPad) }
    };
    this.decorationTypes.set(key, vscode2.window.createTextEditorDecorationType(decOpts));
    return key;
  }
  ...
  getDecorationType(type) {
    return this.decorationTypes.get(type);
  }
  getAllDecorationTypes() {
...
var Decorator = class {
  ...
  updateConfig(enabled) {
    ...
    if (vscode3.window.activeTextEditor) {
      this.applyDecorations(vscode3.window.activeTextEditor);
    }
  }
  applyDecorations(editor) {
    ...
    const ranges = this.parseManager.getRanges(document3);
    if (!ranges) {
      return;
    }
    const visibleRanges = editor.visibleRanges;
    const selections = editor.selections;
    const categorizedRanges = /* @__PURE__ */ new Map();
    ...
    for (const range of ranges) {
      ...
      const vsRange = new vscode3.Range(
    ...
    for (const [type, typeRanges] of categorizedRanges.entries()) {
      ...
      if (decorationType) {
        editor.setDecorations(decorationType, typeRanges);
      }
    }
...
${url}`) : url
  ...
  resolveImageUri(document3, url) {
    try {
      if (url.startsWith("http")) {
        return vscode3.Uri.parse(url);
      }
      return vscode3.Uri.joinPath(vscode3.Uri.joinPath(document3.uri, ".."), url);
    } catch {
      return void 0;
    }
  }
...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode4.window.onDidChangeTextEditorVisibleRanges((e) => {
      updateView(e.textEditor);
    })
  );
  const initialEnabled = vscode4.workspace.getConfiguration().get("obsidianMdInline.hideSyntaxMarkers") ?? true;
